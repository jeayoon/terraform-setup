...
def main(args):
    ...
    if params.workload_manager == "slurm":
        ...
        # Added by Leo
        # Install Apptainer
        if Config.enable_apptainer:
            ExecuteBashScript("./utils/install_apptainer.sh").run()

        # Added by Leo
        # Install Conda
        if Config.enable_conda:
            ExecuteBashScript("./utils/install_conda.sh").run()

        # Added by Leo
        # Install Paramiko
        if Config.enable_paramiko:
            if node_type == SlurmNodeType.HEAD_NODE:
                ExecuteBashScript("./utils/install_paramiko.sh").run()
